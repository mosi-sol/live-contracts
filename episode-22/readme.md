...
contract HashMessage {
   ...
   function _hashContent(string calldata txt) internal view returns (bytes32 x) {
    ...
    function hashTo(string calldata to) external view returns (bytes16) {
        return _hashGen(to);
    }

    function hashTxt(string calldata txt) external view returns (bytes32) {
        return _hashContent(txt);
    }
}
